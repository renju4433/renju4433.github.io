...
function setMaxGuesses() {
    localStorage.setItem('guesses' + bot.type, 21);
}
...
function writeBestGuessList(guesses) {
    ...
    for (let i = 0; i < list_length; i++) {
        let num_guesses = (guesses[i].average - guessesMadeSoFar()).toFixed(4);
...
function calculateGuessList(answers, guesses, best_words) {
    ...
    for (let i = 0; i < best_words.length; i++) { 
        ...
        if (shouldStopTesting(start_time, performance.now(), can_finish, i)) {
            console.log("only calculated " + (i+1) + " words in " + ((performance.now()-start_time)/1000).toFixed(4) + " seconds");
