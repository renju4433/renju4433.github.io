    ...
    if (c >= '0' && c <= '9') {
      ...
      if (offset !== -1) {
        for (var j = 0; j < nums.length; j++) {
          var isZero = (str[i - nums.length + j] === '0');
          if (offset === 27 && isZero) continue;
...
function getShanten(counts, count, type) {
  var minS = 8;
  ...
  if (type === 'standard') return normalMinS;
  return Math.min(minS, normalMinS);
}